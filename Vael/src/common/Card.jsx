...
const Card = ({ products ,limit=products.length ,cols=5 ,key}) => {
...
const gridMap = {
  ...
  return (
    <div className='w-full px-14 py-7' >
...
<Link    to={`/product/${item.id}`}    >
              ...
              {item.isNew  && <span className='absolute top-3 left-3 bg-ink text-white text-[9px] font-semibold tracking-widest uppercase px-2 py-1'>New</span>}
...
<Link    to={`/product/${item.id}`}    >
            ...
            <div className='px-4 py-4 flex flex-col gap-1.5 relative'>
              ...
              {item.isSale && <span className='absolute top-2 right-0 bg-green-500 text-white text-[12px] font-semibold tracking-widest uppercase  px-2 py-1 rounded-l-xl'>−{item.discount}%</span>}
