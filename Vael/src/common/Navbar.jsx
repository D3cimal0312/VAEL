     ...
     <NavLink to='/'> <Logo /></NavLink>
          ...
          <NavLink to={getPath(item)} key={index}>
            {({ isActive }) => (
              <div className="linked tracking-widest group cursor-pointer">
                <p className={`-mb-1 text-2xl transition-colors duration-200 ${isActive ? 'text-ink' : 'text-slate-500 hover:text-ink'}`}>
                  {item}
                </p>
                <div className={`h-0.5 bg-lux transition-all duration-300 ease-in-out ${isActive ? 'w-full opacity-100' : 'w-0 opacity-0 group-hover:w-full group-hover:opacity-100'}`} />
              </div>
            )}
          </NavLink>
        ))}
      ...
      <div className="flex gap-8 items-center">
        ...
        <div className="relative">
          ...
          <div className="absolute -top-5 -right-2 bg-lux rounded-full px-2 text-sm text-white">3</div>
        </div>
      </div>
    </div>
  )
