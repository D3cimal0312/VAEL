...
const Card = ({ products, limit = products.length, cols = 5 }) => {
  ...
  const handleQuickAdd = async (item) => {
    ...
    if (!defaultColor) toast.error("No color available");
    if (!defaultSize) toast.error("No size available");
      ...
      <div className={`grid ${gridMap[cols] || gridMap[5]} gap-8`}>
        {products.slice(0, limit).map((item, index) => (
          <div key={index} data-aos="fade-up">
            <div className="  flex flex-col bg-cream rounded-b-lg group hover:shadow-2xl hover:-translate-y-2 transition-all duration-300 ease-linear h-116">
              <div className="relative overflow-hidden">
                ...
                {item?.isNewArrival && (
                  <span className="absolute top-3 left-3 bg-ink text-white text-[9px] font-semibold tracking-widest uppercase px-2 py-1">
                    New
                  </span>
                )}
                ...
                <div className="absolute top-3 right-3 flex flex-col gap-2 translate-x-14 group-hover:translate-x-0 transition-all duration-300">
                  <button
                    ...
                    className="bg-white text-ink w-8 h-8 rounded-full flex items-center justify-center text-sm shadow-md hover:bg-lux hover:text-white transition-all duration-300"
                  >
                    <Heart
                      size={22}
                      fill={isFavourite(item._id) ? "#d4905a" : "white"}
                      stroke={isFavourite(item._id) ? "none" : "black"}
                    />
                  </button>
                </div>
              ...
              <Link to={`/products/${item.slug}`}>
                <div className="px-4 py-4 flex flex-col gap-1.5 relative">
                  <h3 className="font-fair text-lg tracking-widest ">
                    {item.name}
                  </h3>
                  {item.isSale && (
                    <span className="absolute top-2 right-0 bg-green-500 text-white text-[12px] font-semibold tracking-widest uppercase  px-2 py-1 rounded-l-xl">
                      −
                      {Math.floor(
                  ...
                  <div className="font-fair font-semibold text-xl">
                    {item.originalPrice ? (
                      <div className="flex gap-4 items-center">
                        <p className="text-hair line-through text-xl">
                          ${item.originalPrice}
                        </p>
